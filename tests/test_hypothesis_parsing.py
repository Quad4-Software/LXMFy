...
class TestParsingPropertyBased:
    ...
    @given(
        prefix=st.text(min_size=1, max_size=5).filter(lambda x: not any(s.isspace() for s in x)),
        cmd=st.text(min_size=1, max_size=20).filter(lambda x: not any(s.isspace() for s in x)),
        args=st.lists(st.text(min_size=1, max_size=20).filter(lambda x: not any(s.isspace() for s in x)), min_size=0, max_size=10)
    )
    ...
    @given(
        ...
        args=st.lists(st.text(min_size=1).filter(lambda x: not any(s.isspace() for s in x)))
    )
