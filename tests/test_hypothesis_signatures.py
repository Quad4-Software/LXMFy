...
import RNS
import pytest
from lxmfy.signatures import SignatureManager, FIELD_SIGNATURE
...
class TestSignaturePropertyBased:
    ...
    def test_canonicalize_roundtrip_consistency(self, source_hash, dest_hash, content, title, timestamp, fields):
    ...
    def test_signature_verification_property(self, content, title):
        ...
        assert sig_manager.verify_message_signature(
            mock_message,
            signature,
            sender_hash,
            identity
    ...
    @given(
        content=st.binary(min_size=1, max_size=500),
        invalid_sig=st.binary(min_size=1, max_size=64) # Ed25519 sigs are 64 bytes, but testing variety
    )
    def test_signature_fails_with_wrong_data(self, content, invalid_sig):
        """Test that invalid signatures or modified data fail verification."""
        bot = mock.MagicMock()
        sig_manager = SignatureManager(bot)
        identity = RNS.Identity()
        ...
        mock_message.destination_hash = b"dest"
        mock_message.content = content
        mock_message.title = b"title"
        mock_message.timestamp = 123456789
        mock_message.fields = {}
        
        sender_hash = RNS.hexrep(identity.hash, delimit=False)
        
        # Verify that random bytes don't (usually) verify as valid signatures
