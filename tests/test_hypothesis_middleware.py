...
class TestMiddlewarePropertyBased:
    ...
    def test_message_tracker_pruning(self, hashes, max_size):
        ...
        # The number of tracked hashes should never exceed max_size (roughly)
        # Note: MessageTracker.is_processed uses set(list(self.processed)[-self.max_size:])
        # which might be slightly off due to set ordering, but it ensures size limit.
        assert len(tracker.processed) <= max_size
        
        # Last added items should generally be in the set
        last_item = hashes[-1]
        assert last_item in tracker.processed

    @given(
        h=st.text(min_size=1)
    )
