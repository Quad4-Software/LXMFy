...
class TestNLPManifolds:
    ...
    def test_intent_orthogonality_matrix(self):
        ...
        intents = {
            "shutdown": ["turn off the bot", "shutdown now", "stop the service", "kill process"],
            "status": ["what is the status", "is the bot running", "check service health", "is it alive"],
            "restart": ["restart the bot", "reboot service", "turn off and on again", "refresh process"],
            "help": ["how do i use this", "show me commands", "help me please", "what can you do"]
        }
        ...
        # Report collisions
        if collisions:
            msg = "\n".join([f"Ambiguous intent pair: {n1} <-> {n2} (Sim: {s:.4f})" for n1, n2, s in collisions])
        ...
        for n1, n2, s in collisions:
            assert s < 0.9, f"Intents '{n1}' and '{n2}' are mathematically indistinguishable (Sim: {s})"
    ...
    def test_vector_space_density(self):
        ...
        nlp_dense.add_intent("help", ["help", "i need help", "assist me", "show commands", "manual"])
